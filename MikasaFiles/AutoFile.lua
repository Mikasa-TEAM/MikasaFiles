...
DevAbs:del(Mikasa.."Abs:Lock:AutoFile")
end
...
DevAbs:set(Mikasa.."Abs:Lock:AutoFile",true) 
end 
end

if (text and not DevAbs:get(Mikasa.."Abs:Lock:AutoFile")) then
Time = DevAbs:get(Mikasa.."Abs:AutoFile:Time")
if Time then 
...
BasicConstructors = DevAbs:smembers(Mikasa..'Abs:BasicConstructor:'..v)
Constructors = DevAbs:smembers(Mikasa..'Abs:Constructor:'..v)
...
local curl = 'curl "' .. abbas .. '" -F "chat_id='..DevId..'" -F "document=@'..Mikasa..'.json' .. '" -F "caption=⌁︙يحتوي الملف على ↫ '..#list..' مجموعه"'
io.popen(curl)
io.popen('fm -fr '..Mikasa..'.json')
DevAbs:set(Mikasa.."Abs:AutoFile:Time",os.date("%x"))
end
else 
DevAbs:set(Mikasa.."Abs:AutoFile:Time",os.date("%x"))
end
